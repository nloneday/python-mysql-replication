...
class TestDataType(base.PyMySQLReplicationTestCase):
    ...
    def test_decimal(self):
        ...
        self.assertEqual(event.rows[0]["values"]["test"], format(Decimal("4.2"), '.1f'))
    ...
    def test_decimal_long_values(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], format(Decimal("42000.123456"), '.10f'))
    ...
    def test_decimal_long_values_1(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], format(Decimal("9000000123.123456"), '.10f'))
    ...
    def test_decimal_long_values_2(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], format(Decimal("9000000123.0000012345"), '.10f'))
    ...
    def test_decimal_two_values(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], format(Decimal("4.2"), '.1f'))
        self.assertEqual(event.rows[0]["values"]["test2"], format(Decimal("42000.123456"), '.10f'))
    ...
    def test_decimal_with_zero_scale_1(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], format(Decimal("10"), '.0f'))

    def test_decimal_with_zero_scale_2(self):
        create_query = "CREATE TABLE test (test DECIMAL(23,0))"
        insert_query = "INSERT INTO test VALUES(12345678912345678912345)"
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], format(Decimal("12345678912345678912345"), '.0f'))

    def test_decimal_with_zero_scale_3(self):
        create_query = "CREATE TABLE test (test DECIMAL(23,0))"
        insert_query = "INSERT INTO test VALUES(100000.0)"
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], format(Decimal("100000"), '.0f'))
    ...
    def test_decimal_with_zero_scale_6(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], format(Decimal("-1234567891234567891234"), '.0f'))
    ...
    def test_timestamp(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], '1984-12-03 12:33:07')
    ...
    def test_timestamp_mysql56(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test0"], '1984-12-03 12:33:07')
        self.assertEqual(event.rows[0]["values"]["test1"], '1984-12-03 12:33:07.100000')
        self.assertEqual(event.rows[0]["values"]["test2"], '1984-12-03 12:33:07.120000')
        self.assertEqual(event.rows[0]["values"]["test3"], '1984-12-03 12:33:07.123000')
        self.assertEqual(event.rows[0]["values"]["test4"], '1984-12-03 12:33:07.123400')
        self.assertEqual(event.rows[0]["values"]["test5"], '1984-12-03 12:33:07.123450')
        self.assertEqual(event.rows[0]["values"]["test6"], '1984-12-03 12:33:07.123456')
    ...
    def test_date(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], '1984-12-03')
    ...
    def test_zero_year(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], None)
        self.assertEqual(event.rows[0]["values"]["test2"], '0000-01-21')
    ...
    def test_zero_month(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], None)
        self.assertEqual(event.rows[0]["values"]["test2"], '2015-00-21')
    ...
    def test_zero_day(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], None)
        self.assertEqual(event.rows[0]["values"]["test2"], '2015-05-00')

    def test_time(self):
        create_query = "CREATE TABLE test (test1 TIME, test2 TIME);"
        insert_query = "INSERT INTO test VALUES('838:59:59', '-838:59:59')"
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test1"], '838:59:59')
        self.assertEqual(event.rows[0]["values"]["test2"], '-838:59:59')

    def test_time2(self):
        if not self.isMySQL56AndMore():
            self.skipTest("Not supported in this version of MySQL")
        create_query = "CREATE TABLE test (test1 TIME(3), test2 TIME(6));"
        insert_query = """
            INSERT INTO test VALUES('17:51:04.123456', '-838:59:58.999999');
        """
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test1"], '17:51:04.123000')
        self.assertEqual(event.rows[0]["values"]["test2"], '-838:59:58.999999')

    def test_zero_time(self):
        create_query = "CREATE TABLE test (id INTEGER, test TIME NOT NULL DEFAULT 0);"
        insert_query = "INSERT INTO test (id) VALUES(1)"
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], '00:00:00')
    ...
    def test_zero_datetime(self):
        self.set_sql_mode()
        create_query = "CREATE TABLE test (id INTEGER, test DATETIME NULL, test2 DATETIME NOT NULL DEFAULT 0);"
        insert_query = "INSERT INTO test (id) VALUES(1)"
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], None)
        self.assertEqual(event.rows[0]["values"]["test2"], '0000-00-00 00:00:00')
    ...
    def test_broken_datetime(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], '2013-00-00 00:00:00')
    ...
    def test_varchar(self):
        create_query = "CREATE TABLE test (test VARCHAR(242), test2 VARCHAR(242)) CHARACTER SET utf8 COLLATE utf8_general_ci;"
        insert_query = "INSERT INTO test VALUES('Hello', '你好')"
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], 'Hello')
        self.assertEqual(event.rows[0]["values"]["test2"], '你好')
        self.assertEqual(event.columns[0].max_length, 242 * 3)
    ...
    def test_enum(self):
        create_query = "CREATE TABLE test (test ENUM('a', 'ba', 'c'), test2 ENUM('a', 'ba', '你好')) CHARACTER SET utf8 COLLATE utf8_general_ci;"
        insert_query = "INSERT INTO test VALUES('ba', '你好')"
        event = self.create_and_insert_value(create_query, insert_query)
    ...
    def test_set(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["test"], ','.join(('a', 'ba', 'c')))
        self.assertEqual(event.rows[0]["values"]["test2"], ','.join(('a', 'c')))
    ...
    def test_json_unicode(self):
        ...
        event = self.create_and_insert_value(create_query, insert_query)
        self.assertEqual(event.rows[0]["values"]["value"], {"miam": "🍔"})
...
def test_suite():
    ...
    suite.addTest(TestDataType('test_decimal'))
    suite.addTest(TestDataType('test_decimal_long_values'))
    suite.addTest(TestDataType('test_decimal_long_values_1'))
    suite.addTest(TestDataType('test_decimal_long_values_2'))
    suite.addTest(TestDataType('test_decimal_negative_values'))
    suite.addTest(TestDataType('test_decimal_two_values'))
    suite.addTest(TestDataType('test_decimal_with_zero_scale_1'))
    suite.addTest(TestDataType('test_decimal_with_zero_scale_2'))
    suite.addTest(TestDataType('test_decimal_with_zero_scale_3'))
    suite.addTest(TestDataType('test_decimal_with_zero_scale_4'))
    suite.addTest(TestDataType('test_decimal_with_zero_scale_6'))
    suite.addTest(TestDataType('test_timestamp'))
    suite.addTest(TestDataType('test_timestamp_mysql56'))
    suite.addTest(TestDataType('test_date'))
    suite.addTest(TestDataType('test_zero_year'))
    suite.addTest(TestDataType('test_zero_month'))
    suite.addTest(TestDataType('test_zero_day'))
    suite.addTest(TestDataType('test_time'))
    suite.addTest(TestDataType('test_time2'))
    suite.addTest(TestDataType('test_zero_time'))
    suite.addTest(TestDataType('test_datetime'))
    suite.addTest(TestDataType('test_zero_datetime'))
    suite.addTest(TestDataType('test_broken_datetime'))
    suite.addTest(TestDataType('test_varchar'))
    suite.addTest(TestDataType('test_json'))
    suite.addTest(TestDataType('test_set'))
    suite.addTest(TestDataType('test_json_array'))
    suite.addTest(TestDataType('test_json_large'))
    suite.addTest(TestDataType('test_json_types'))
    suite.addTest(TestDataType('test_json_basic'))
    suite.addTest(TestDataType('test_json_unicode'))
    suite.addTest(TestDataType('test_json_long_string'))
    suite.addTest(TestDataType('test_encoding_utf8'))
    return suite
